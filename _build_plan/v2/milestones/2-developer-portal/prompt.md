...
You are entering plan mode to plan and then build milestone 2 of the WaGuard v2 build phase.

## Context

- Read `@_build_plan/v2/prd.md` for the full v2 project context, scope, data model, and tech stack.
- Read previous v2 milestone folders (`@_build_plan/v2/milestones/1-*/milestone-log.md`) to understand what has already been built. The original v1 build lives in `@_build_plan/` for reference.

## Your task

1. Plan the implementation for **only** milestone 2 as defined in the v2 PRD. Do not plan or build anything from later milestones.
2. After the user confirms the plan, build only what is in milestone 2's scope.
3. Verify your work against the "Done when" criteria for milestone 2 in the v2 PRD.
4. When complete, write a `milestone-log.md` in this folder (`_build_plan/v2/milestones/2-developer-portal/milestone-log.md`). Structure it as follows:
