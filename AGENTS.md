...
The `_build_plan/` folder contains the PRDs and per-milestone prompts used to scaffold this codebase during its build-out phases — `_build_plan/` (the initial v1 build) and `_build_plan/v2/` (the second phase: dashboard redesign, developer portal, and the deferred functional features). These files are **temporary** — they exist for documentation and guidance only. They are **not** functional: no code, configuration, or runtime logic in this codebase should import, reference, or depend on anything inside `_build_plan/`.
